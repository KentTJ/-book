...
ams： 责Android四大组建的启动和管理，进程的创建和调度。
...
> > linux：进程活动停止后就结束该进程
> >
> > 安卓：不会立即结束进程（保存在内存中），直到（系统处于低内存的状态下，需要更多内存）
>
...
向linux侧oom_adj的设置（类似API）：

> 向   /proc/进程id/oom_adj文件中写入对应的oom_adj值
>
> **本质： 进程id ------oom_adj 的映射**

## 进程管理的基础-------Linux cgroups
...
1. cat proc/[pid]/oom_score_adj -------> 为啥会写进文件里？不应该是实时的嘛？这里是整数，比如-900
2. adb shell dumpsys activity o/p

例子：

> myapplication分别在前台和后台时ADJ值
