...
```java
  UID ------  用户id， 1000(chengang)
  GID ------  用户组id， 1000(chengang)
```
...
```java
  $ id
  uid=1000(chengang) gid=1000(chengang) 组=1000(chengang),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),116(lpadmin),126(sambashare)
```
...
```java
  chengang@chengang-VirtualBox:/proc/2160/task/2750$ cat /etc/passwd
  ...
  chengang:x:1000:1000:chengang,,,:/home/chengang:/bin/bash  // ---------> chengang用户，用户组也是1000:chengang
...
```java
  chengang@chengang-VirtualBox:/proc/2160/task/2750$ cat /etc/group
  root:x:0:
  daemon:x:1:
  bin:x:2:
  sys:x:3:
  adm:x:4:syslog,chengang
  tty:x:5:
  disk:x:6:
  lp:x:7:
  mail:x:8:
